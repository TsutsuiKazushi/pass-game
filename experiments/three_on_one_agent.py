...
class ThreeOnOne:
    ...
    def __init__(self, accel_defender, accel_attacker1, accel_attacker2, accel_attacker3, accel_ball=3, \
                 pass_noise1=0, pass_noise2=0, pass_noise3=0, threshold_ang2=30, threshold_ang3=30, pass_bias2=0, pass_bias3=0, \
                 mass_defender=1,  mass_attacker1=1, mass_attacker2=1, mass_attacker3=1, mass_ball=0.3, \
                 damping=0.25, dt=0.1, max_step=9999, threshold_max=50, sigma=0, pass_prob=1):
        ...
        self.pass_noise1 = pass_noise1
        self.pass_noise2 = pass_noise2
        self.pass_noise3 = pass_noise3
        self.threshold_ang2 = threshold_ang2
        self.threshold_ang3 = threshold_ang3
        self.pass_bias2 = pass_bias2
        self.pass_bias3 = pass_bias3
        self.threshold_max = threshold_max
        self.sigma = sigma
        self.rng = np.random.default_rng(sigma)
    ...
    def reset(self):
        ...
        rand = 3 # initial positions are fixed
        ang1 = rand * -np.pi / 6
    ...
    def step(self, obs_d, obs_a1, obs_a2, obs_a3, action_d, action_a1, action_a2, action_a3, num_step):
        ...
        # ==== For agent ==== 
        abs_u_a1, abs_u_b_a1, next_pass_a1, next_with_b_a1, action_a1 = get_abs_ball_u_red(action_a1, self.pos_a1, self.pos_a2, self.pos_a3, self.pos_d, self.with_b_a1, self.pass_a1, self.to_a1, self.to_a2, self.to_a3, self.pos_b, self.vel_b, self.pass_noise2, self.threshold_ang2, self.pass_bias2, self.threshold_max, self.sigma, self.dt, self.rng, self.pass_prob)
        # =================
                
        abs_u_a2, abs_u_b_a2, next_pass_a2, next_with_b_a2, action_a2 = get_abs_ball_u(action_a2, self.pos_a2, self.pos_a3, self.pos_a1, self.pos_d, self.with_b_a2, self.pass_a2, self.to_a2, self.to_a3, self.to_a1, self.pos_b, self.vel_b, self.pass_noise2, self.threshold_ang2, self.pass_bias2)
        
        abs_u_a3, abs_u_b_a3, next_pass_a3, next_with_b_a3, action_a3 = get_abs_ball_u(action_a3, self.pos_a3, self.pos_a1, self.pos_a2, self.pos_d, self.with_b_a3, self.pass_a3, self.to_a3, self.to_a1, self.to_a2, self.pos_b, self.vel_b, self.pass_noise3, self.threshold_ang3, self.pass_bias3)
...
def get_abs_ball_u(action, abs_own_pos, abs_mate1_pos, abs_mate2_pos, abs_adv_pos, \
                   ...
                   abs_ball_pos, abs_ball_vel, pass_noise, threshold_ang, pass_bias):
    
    if with_b_own == True:
        
        threshold_defender = 2.0
        threshold_min = threshold_ang
        threshold_bias = pass_bias
        
        dist = get_dist(abs_own_pos, abs_adv_pos)
        
        if dist < threshold_defender:
            a, b = get_angle_pass(abs_own_pos, abs_mate1_pos, abs_mate2_pos, abs_adv_pos)
            a, b = a*180/np.pi, b*180/np.pi
            
            if a < threshold_min and b < threshold_min: 
                abs_u = [0, 0]
                abs_u_b = [0, 0]
                next_pass_own = False
                ...
                if a > b + threshold_bias:
                    abs_u = [0, 0]
                    sub_u_b = [1, 0]
                    noise = np.random.normal(0, pass_noise, 2)
                    sub_u_b += noise
...
def get_abs_ball_u_red(action, abs_own_pos, abs_mate1_pos, abs_mate2_pos, abs_adv_pos, \
                   with_b_own, pass_own, to_own, to_mate1, to_mate2, \
                   abs_ball_pos, abs_ball_vel, pass_noise, threshold_ang, pass_bias, threshold_max, sigma, dt, rng, pass_prob):
    ...
    if with_b_own == True:
        ...
        if dist < threshold_defender:
            ...
            v = a - b
            norm = float(rng.standard_normal())
            evidence = v * dt + sigma * np.sqrt(dt) * norm
            
            pass_rand = rng.random()
            if a < threshold_min and b < threshold_min: 
                abs_u = [0, 0]
                abs_u_b = [0, 0]
                next_pass_own = False
                ...
                bound = 0 + threshold_bias
                sampled = 1 if evidence > bound else 2
